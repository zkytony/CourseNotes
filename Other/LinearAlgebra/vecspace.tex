...
%% To find the change-of-basis matrix $\bm{P}$, we express every basis vector in $\mathcal{B}_1$ as a linear combination of basis vectors in $\mathcal{B}_2$. Namely,
%% \begin{align}
%%   \bm{u_1}&=p_{11}\bm{v}_1+\cdot+p_{n1}\bm{v}_n=\sum_{k=1}^n\bm{v_k}\bm{p_{|k}}^T\\
%%   &\vdots\\
%%   \bm{u_n}&=p_{1n}\bm{v}_1+\cdot+p_{nn}\bm{v}_n
%% \end{align}
%% Every equation involves $n$ unknowns ($p_{1i}\cdots p_{ni}$) and $n$ equations (since every vector is $n$-dimenstional). We can solve all of them to obtain all the coefficients, which form matrix $P$. Notice the coefficients found by solving each equation are the column vectors $\bm{p_k}$ of $P$ (as indicated by the indices).

%% See an example at \url{https://www.math.hmc.edu/calculus/tutorials/changebasis/}.
